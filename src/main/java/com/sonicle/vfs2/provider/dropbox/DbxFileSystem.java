...
public class DbxFileSystem extends AbstractFileSystem implements FileSystem {
	...
	public DbxClientWrapper getClientWrapper() throws IOException {
		return DbxClientWrapper.getClientWrapper((GenericFileName)getRoot().getName(), getFileSystemOptions());
	}
	
	public void putClientWrapper(DbxClientWrapper client) {
		DbxClientWrapper.releaseClientWrapper(client);
	}

	@Override
	protected FileObject createFile(AbstractFileName afn) throws Exception {
		return new DbxFileObject(afn, this);
	}
